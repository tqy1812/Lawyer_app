...
class DateItem extends PureComponent {
  render() {
    ...
    const highlightBgColor = '#007afe';
    const normalBgColor = 'white';
    const hightlightTextColor = '#fff';
    const normalTextColor = 'rgba(0,0,0,1)';
    return (
          ...
          <View style={[
            ...
            {marked &&
              <View style={[
                ...
                {backgroundColor: highlight ? 'white' : '#007afe'}
              ]} />
            }
          </View>
        </TouchableOpacity>
...
class CalendarStrip extends Component {
    ...
    if (!this.currentPageDatesIncludes(nextSelectedDate)) {
      ...
      if (this.state.datas.find(sameDay)) {
        ...
        if (isFuture(nextSelectedDate)) {
          ...
          const days = eachDayOfInterval({start: head, end: tail});
          this.setState({
            datas: days,
  ...
  getInitialDates(weekStartsOn=0) {
    ...
    const eachDays = eachDayOfInterval({start: startLast2Week, end: endNext2Week});
    return eachDays;
  }
  ...
  loadNextTwoWeek(originalDates) {
    ...
    const eachDays = eachDayOfInterval({start: originalFirstDate, end: lastDayOfNext2Week});
  ...
  loadPreviousTwoWeek(originalDates) {
    ...
    const eachDays = eachDayOfInterval({start: firstDayOfPrevious2Week, end: originalLastDate});
  ...
  _renderHeader = () => {
    ...
    const dateFormatted_zh = format(selectedDate, 'yyyy年 M月', {locale: ChineseLocale});
